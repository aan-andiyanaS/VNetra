# Formula Matematis — Sistem Navigasi Bantu Tunanetra (Revisi v9.4)

> **Dokumen ini adalah revisi dari v9.3** dengan serangkaian koreksi bertahap hasil audit menyeluruh (lihat entri [DIPERBARUI] dan [Post-audit] di bawah):
>
> **[DIPERBARUI v9.4]** Audit konsistensi notasi menyeluruh — 11 inkonsistensi diperbaiki:
> - **[K-1]** Footer: nomor versi diubah dari `9.1` ke `9.4`, tanggal diperbarui ke 2026-06-05
> - **[K-2]** Konstanta Sistem: `$R_{th}$` → `$R_{th,hi}$` agar simetris dengan `$R_{th,lo}$` dan konsisten dengan notasi Formula J.3
> - **[K-3]** Formula F.5: label pool `MID` diseragamkan ke `MEDIUM` (nama resmi yang didefinisikan di F.4)
> - **[K-4]** Formula J.3: tipe terrain `CONTAM.` diseragamkan ke `CONTAMINATED` agar cocok dengan J.7
> - **[S-1]** Diagram Ringkasan — paket WebSocket dikoreksi dari 6 field ke 9 field sesuai spesifikasi A.6
> - **[S-2]** Formula G.EKF: klarifikasi bahwa $\|\mathbf{P}_t\|_F$ **dihitung di ESP32**, tidak dikirim mentah — hanya `is_converged` yang dikirim
> - **[S-3]** Tabel A.2: keterangan `$\omega_z^{corr}$` dikoreksi — tidak digunakan Formula I atau K
> - **[M-1]** Diagram A.5: nama zona J diperbarui dari `R_low/R_mid` (v7) ke `r_low/r_mid/r_high/r_ultra` (v8)
> - **[M-2]** Formula G.2: superscript per-objek `(i)` ditambahkan ke $v_{raw}$, $d_{obj}$, $v_{head}$
> - **[M-3]** Formula J.2 Fitur 3: catatan klarifikasi $N_{row,low} = 1$ (implisit) ditambahkan
> - **[M-4]** Diagram Ringkasan: `gz` dikoreksi ke `ωz` sesuai rename v9.1
> - **[Post-audit A1]** Diagram Ringkasan — blok Formula I diperbarui dari representasi v7 (single ΔA) ke arsitektur TTC Multi-Dimensional v8 (area/ar/dist score + tiga pool IMMINENT/PROBABLE/POSSIBLE)
> - **[Post-audit A2]** Formula F.5: satu instans "pool MID" yang tersisa diubah ke "pool MEDIUM" (melengkapi [K-3])
> - **[Post-audit A3]** Tabel F.2: referensi `Formula A.5` dikoreksi ke `Formula A.EKF.4` (lokasi perhitungan $\|\mathbf{a}_{lin}\|$ yang benar)
> - **[Post-audit A4]** Formula E.3: deskripsi verbal arah geser $r_{center}$ dikoreksi — saat menunduk ($\theta > 0$), $r$ bertambah (ke baris lebih besar/bawah), bukan ke baris lebih kecil/atas
> - **[Post-audit A5]** Formula J.0: guard non-overlap zona vertikal diformalkan menjadi tiga persamaan eksplisit ($r_{mid} := \max(r_{mid}, r_{high}+1)$, dst.) menggantikan deskripsi verbal saja
> - **[Post-audit A6]** Footer arsitektur: posisi "K fail-safe" dipindahkan ke dalam klausul ESP32-S3 agar tidak membingungkan pembaca bahwa K berjalan di smartphone
> - **[Post-audit A7]** Seluruh 55 heading sub-bab (A.1–K.5) diubah dari skema `X.N Title` ke `X — Title` untuk menghilangkan ambiguitas penomoran dengan label sub-formula internal (E.2, F.2–F.5, G.2–G.4, I.2–I.5, J.2–J.5, K.2–K.3)
> - **[Post-audit A8]** Formula E: Range output $d_{obj}$ dikoreksi dari $[0,4000]$ ke $[\varepsilon_{noise}, d_{max}]$ — nilai 0 tidak mungkin muncul setelah filter E.0
> - **[Post-audit A9]** Tabel G — Keterangan Variabel: satuan $v_{head}^{(i)}$ ditambahkan penjelasan dimensi (rad bersifat dimensionless, sehingga rad/s × mm = mm/s)
> - **[Post-audit A10]** Tabel contoh numerik G.3: catatan ditambahkan bahwa setiap baris adalah skenario per-objek independen dengan $d_{obj}^{(i)}$ berbeda
> - **[Post-audit B1]** Header v9.4: frasa "dua koreksi" diubah ke "serangkaian koreksi bertahap" agar sesuai dengan jumlah koreksi aktual (17+)
> - **[Post-audit B2]** Formula E baris Range output: double-write `[0, 4000]` yang tersisa dihapus
> - **[Post-audit B3]** Formula E Domain input: ditambahkan `∪ {65535}` untuk mencakup sentinel error sensor VL53L5CX
> - **[Post-audit B4]** Tabel variabel E: domain `d_obj` diperbarui dari `[0, 4000]` ke `[ε_noise, d_max]`
> - **[Post-audit B5]** Formula F.4: "debounce 2 detik" diganti "throttle via f_tts (min. 1 detik)" agar konsisten dengan Konstanta Sistem
> - **[Post-audit B6]** Formula K: ditambahkan K.1 (himpunan valid V_K) dan K.1b (min dengan fallback d_max) untuk mencegah buzzer palsu akibat nilai error 0/65535 — KRITIKAL
> - **[Post-audit B7]** Formula J narasi v8: "lima tipe terrain" dikoreksi menjadi "tujuh tipe terrain" (+ OPEN + CONTAMINATED)
> - **[Post-audit B8]** Formula J contoh estimasi: TTS "15 sentimeter" dikoreksi menjadi "1,5 meter" (~155 cm) — KRITIKAL
> - **[Post-audit B9]** Formula I routing PROBABLE: "Kirim ke Formula G" → "Kirim ke Formula G dan H" agar konsisten dengan diagram
> - **[Post-audit B10]** Tabel G.3 baris 950mm: kolom "Objek di" dikoreksi dari `2500 ≥ 1980` ke `950 < ~1980`
> - **[Post-audit B11]** Tabel Ringkasan Formula G: kolom Input diperbarui dari `ω_x^corr` ke `v_head_base, is_converged` sesuai arsitektur v8
> - **[Post-audit C1]** Tabel variabel F, G, H: domain `d_obj` dari `[0, 4000]` ke `[\varepsilon_{noise}, d_max]` — konsisten dengan post-audit B4 di tabel E
> - **[Post-audit C2]** Formula F.4: routing MEDIUM ke G **dan H** — konsisten dengan diagram aliran F
> - **[Post-audit C3]** Formula J.7 pseudocode: STAIR_UP ditambahkan ke set HIGH — menutup blind spot keselamatan saat confidence > C_HIGH
> - **[Post-audit C4]** Tabel Ringkasan & Diagram Aliran J: 5 tipe terrain -> 7 tipe (+ OPEN + CONTAMINATED)
> - **[Post-audit C5]** Fitur 2 J.2 (Δz_t): ditambahkan klarifikasi penggunaan di J.3 & J.6 sebagai modifier confidence HOLE
> - **[Post-audit C6]** Formula J.1: z̄_mid kini menggunakan {r_mid, r_mid+1} — menghindari overlap dengan zona r_high
> - **[Post-audit C7]** Formula F.3b: ditambahkan klarifikasi bahwa Δt sudah melalui sat() dari G.0
> - **[Post-audit C8]** Tabel variabel E, J, K: domain z_rj -> [0,d_max] ∪ {65535} (sentinel error sensor)
> - **[Post-audit C9]** Tabel variabel K: domain d_min -> [ε_noise, d_max] sesuai filter K.1
> - **[Post-audit C10]** Formula I.3: guard degenerate BBox diperluas ke x_max=x_min — mencegah λ=0/0=NaN di pipeline I
> - **[Post-audit C11]** Formula J.7 pseudocode: routing MED diperbarui ke "Formula G dan H" — konsisten dengan diagram Aliran J
> - **[Post-audit D1]** Catatan Arsitektur A (baris notasi): update output WebSocket dari 4 field ke 9 field sesuai A.6 v8.1
> - **[Post-audit D2]** Formula K.3: pernyataan Domain inline diperbarui dari d_min in [0,4000] ke [varepsilon_noise, d_max] -- konsisten dengan tabel variabel K
> - **[Post-audit D3]** Diagram aliran F: anotasi `debounce 2s` MEDIUM diganti `throttle f_tts` -- konsisten dengan teks F.4
> - **[Post-audit D4]** Formula J.1: z_bar_mid menggunakan sat(r_mid+1, r_mid, r_low-1) untuk mencegah overlap baris r_low saat r_mid=4 (max sat)
> - **[Post-audit D5]** Formula J: ditambahkan J.0b filter sentinel (0/65535 → d_max) sebelum rata-rata zona J.1 -- mencegah klasifikasi terrain palsu
> - **[DIPERBARUI v9.3]** Formula E: Contoh postur kepala pada tabel E.1 — baris $\theta = \pm 5.6°$ dikoreksi menjadi $\theta = \pm 5.7°$ (≈ $\pm\delta\theta$); nilai $5.6°$ kurang dari satu langkah $\delta\theta = 5.625°$ sehingga $\text{round}(3.5 + 5.6/5.625) = \text{round}(4.496) = 4$ (bukan 5), sedangkan $5.7°$ menghasilkan $\text{round}(4.513) = 5$ secara deterministik; catatan koreksi ditambahkan setelah tabel
> - **[DIPERBARUI v9.3]** Formula G: Kolom $d_{obj}^{(i)}$ ditambahkan ke tabel contoh numerik G.3 — sebelumnya nilai $v_{head}^{(i)} = v_{head\_base} \times d_{obj}^{(i)}$ tidak dapat diverifikasi pembaca karena $d_{obj}$ tidak dicantumkan di tabel
>
> **[TIDAK BERUBAH dari v9.2]** Konstanta Sistem ($N_{obj,max}$, $f_{tts}$), Formula F (F.4, F.5), Formula G.0 (sub-judul), Formula H (catatan warmup) — tetap sama.
>
> **[TIDAK BERUBAH dari v9.1]** Konstanta $g$, $\omega_{x,lim}$, $\theta_{pitch,lim}$, $N_{warmup}$, $\varepsilon_{conv}$, $\Delta t_{max}$, $N_{v,avg}$ — tetap sama.
>
> **[TIDAK BERUBAH dari v9]** Semua formula A (EKF), B, C, D, F, G, H, I, J, K — tidak ada perubahan substansial pada logika atau struktur formula.
>
> **[TIDAK BERUBAH dari v8]** Formula A.EKF.1 s.d. A.EKF.5 (struktur EKF), Formula B, C, D, F, G, H, I, J, K — tidak ada perubahan substansial.
>
> **[TIDAK BERUBAH dari v7]** Formula A, B, C, D, G, H, K — tetap tidak berubah dari v7 (kecuali perubahan yang tercatat di v9 s.d. v9.3 di atas).
...
| $R_{th,hi}$ | 0.8 | — | Threshold rasio anomali medan (atas) — **[v9.4]** diubah dari $R_{th}$ agar simetris dengan $R_{th,lo}$ dan konsisten dengan Formula J |
...
| $n_{cal}$ | 500 | — | **[BARU v9]** Jumlah sampel kalibrasi statis akselerometer pada fase boot (≈5 detik pada 100 Hz); cukup besar untuk menekan noise acak, cukup singkat agar tidak menunda startup |
| $\mathbf{b}_{acc}$ | (diukur) | m/s² | **[BARU v9]** Vektor bias akselerometer $[b_{ax},\, b_{ay},\, b_{az}]^T$: offset DC pabrik MPU-6050, diukur sekali oleh prosedur A.EKF.0 saat boot, konstanta selama satu sesi operasi |
| $g$ | 9.81 | m/s² | **[v9.1]** Percepatan gravitasi standar; digunakan di A.EKF.0 (normalisasi kalibrasi), A.EKF.3 (fungsi pengukuran $h$), dan A.EKF.4 (perhitungan $\mathbf{g}_q$) |
| $\Delta t_{min}$ | 0.001 | s | Guard minimum interval waktu (1 ms) untuk mencegah division-by-zero |
| $\Delta t_{max}$ | 0.5 | s | **[v9.1]** Guard maksimum interval waktu (500 ms) untuk mencegah underestimate kecepatan saat koneksi WebSocket sempat putus — digunakan di Formula G.0 |
| $\varepsilon_{noise}$ | 30 | mm | Noise floor sensor ToF VL53L5CX; digunakan sebagai threshold reset flag Formula H dan guard filter nilai error sensor E.0 |
| $\omega_{x,lim}$ | 5.0 | °/s | **[v9.1]** Guard laju rotasi pitch ekstrim — jika $|\omega_x^{corr}| > \omega_{x,lim}$, faktor redaman $k_{damp}$ dikurangi ke 0.5 di A.EKF.5 dan G.1a |
| $\theta_{pitch,lim}$ | 30 | ° | **[v9.1]** Batas sudut pitch di mana aproksimasi busur mulai menghasilkan error signifikan ($>15\%$); digunakan sebagai batas validitas keterangan G.1 |
| $N_{warmup}$ | 150 | frame | **[v9.1]** Jumlah frame warmup EKF setelah boot ($\approx$5 detik pada 30 Hz) — selama periode ini `is_converged = 0` dan Formula G tidak aktif |
| $\varepsilon_{conv}$ | 0.10 | — | **[v9.1]** Threshold norma Frobenius kovarians EKF $\|\mathbf{P}_t\|_F$ untuk menyatakan konvergensi — alternatif berbasis matriks dari $N_{warmup}$; digunakan di G.EKF |
| $N_{v,avg}$ | 3 | frame | **[v9.1]** Window moving average kecepatan pendekatan di Formula G.2b; dipilih sebagai titik tengah antara noise rejection dan latensi respons |
...
| $d_{crit}$ | 1000 | mm | **[Formula F.v2]** Jarak kritis (d_score = 1.0 jika $d_{obj} < d_{crit}$). Nilainya sengaja disamakan dengan $d_{w0}$ karena keduanya merepresentasikan "batas keamanan 1 meter" dari perspektif yang berbeda: $d_{w0}$ adalah batas absolut (Formula G, H, K), sedangkan $d_{crit}$ adalah batas saturasi skor (Formula F) |
...
| $N_{obj,max}$ | 3 | — | **[v9.2]** Jumlah maksimum objek yang dimasukkan ke pool peringatan per frame — mencegah TTS overflow saat scene ramai; dipilih berdasarkan kapasitas antrian audio minimal yang dapat diproses dalam 1 siklus 33 ms |
| $f_{tts}$ | 1 | Hz | **[v9.2]** Laju maksimum TTS output untuk pool `MEDIUM` (debounce minimum 1 detik antar peringatan kategori menengah); pool `HIGH` dikecualikan — selalu disuarakan setiap frame tanpa throttle |
...
**A.EKF.0 — Kalibrasi Statis Akselerometer (Boot Phase, sekali jalan)** *[BARU v9]*

Sebelum loop EKF dimulai, ESP32-S3 menjalankan prosedur kalibrasi satu kali saat device diam (misalnya baru dihidupkan di atas meja atau dipakaikan ke pengguna yang berdiri tenang). Prosedur ini mengukur **bias offset pabrik** akselerometer MPU-6050 — kesalahan sistematik yang konstan dan tidak dapat dikurangi oleh EKF karena tidak masuk dalam state vector.

Kumpulkan $n_{cal}$ sampel lalu hitung rata-rata per sumbu:

$$\bar{\mathbf{a}} = \frac{1}{n_{cal}} \sum_{i=1}^{n_{cal}} \mathbf{a}^{(i)}$$

Sensor yang ideal saat diam akan menghasilkan $\|\bar{\mathbf{a}}\| = g = 9.81\ \text{m/s}^2$. Karena bias pabrik, nilai nyata sedikit menyimpang. Normalisasikan rata-rata ke besar gravitasi yang sesungguhnya, lalu hitung selisihnya:

$$\boxed{\mathbf{b}_{acc} = \bar{\mathbf{a}} - \frac{g}{\|\bar{\mathbf{a}}\|}\,\bar{\mathbf{a}} = \bar{\mathbf{a}}\left(1 - \frac{g}{\|\bar{\mathbf{a}}\|}\right)}$$

Interpretasi: $\frac{g}{\|\bar{\mathbf{a}}\|}\,\bar{\mathbf{a}}$ adalah proyeksi vektor rata-rata ke besar gravitasi sebenarnya — apa yang *seharusnya* dibaca sensor. Selisihnya adalah komponen yang tidak bisa dijelaskan oleh gravitasi, yaitu bias. Vektor $\mathbf{b}_{acc}$ ini disimpan sebagai konstanta dan **diaplikasikan di setiap iterasi loop EKF** sebelum pembacaan masuk ke fase koreksi:

$$\boxed{\tilde{\mathbf{a}}_t = \mathbf{a}_t - \mathbf{b}_{acc}}$$

Di mana $\tilde{\mathbf{a}}_t$ adalah pembacaan akselerometer **terkalibrasi** yang digunakan pada A.EKF.3 dan A.EKF.4. Pembacaan raw $\mathbf{a}_t$ tidak lagi digunakan secara langsung setelah langkah ini.

> **Mengapa bias tidak dimasukkan ke state vector EKF?** Secara teoritis bisa (menjadi EKF 10-dimensi dengan $[b_{ax}, b_{ay}, b_{az}]$ tambahan). Namun, bias akselerometer MPU-6050 bersifat sangat lambat berubah (dominan oleh suhu yang berubah dalam hitungan menit, bukan detik), sehingga **kalibrasi statis sekali-jalan lebih efisien** daripada menambah tiga state baru yang memperbesar matriks dari $7\times7$ menjadi $10\times10$ dan melipatgandakan biaya komputasi fase koreksi.
...
Fase ini menggunakan data akselerometer **terkalibrasi** $\tilde{\mathbf{a}}_t = \mathbf{a}_t - \mathbf{b}_{acc}$ — bukan raw $\mathbf{a}_t$ — untuk mengoreksi tebakan dari fase prediksi. Penggunaan $\tilde{\mathbf{a}}_t$ memastikan bahwa saat device diam, inovasi $\mathbf{y}_t$ konvergen ke $\mathbf{0}$ dan keluaran $\|\mathbf{a}_{lin}\|$ konvergen ke $0\ \text{m/s}^2$, bukan ke nilai bias residual $\sim\!0.4\ \text{m/s}^2$.
...
Inovasi (selisih antara prediksi gravitasi dan akselerometer terkalibrasi):

$$\mathbf{y}_t = \tilde{\mathbf{a}}_t - h\!\left(\hat{\mathbf{x}}_{t|t-1}\right)$$
...
$$\boxed{\omega_z^{corr} = \omega_z - b_{\omega z}^{(t)}}$$
...
Magnitude akselerasi linear murni (gravitasi dikurangi secara tepat via quaternion, lebih akurat dari aproksimasi skalar CF; bias akselerometer sudah dihilangkan oleh A.EKF.0 sehingga nilai ini konvergen ke $0\ \text{m/s}^2$ saat device diam):

$$\mathbf{g}_q = \begin{bmatrix} 2(q_x q_z - q_w q_y) \\ 2(q_w q_x + q_y q_z) \\ q_w^2 - q_x^2 - q_y^2 + q_z^2 \end{bmatrix} \cdot g$$

$$\boxed{\|\mathbf{a}_{lin}\| = \left\|\tilde{\mathbf{a}}_t - \mathbf{g}_q\right\|}$$
...
**Domain:**
- Input: $a_x, a_y, a_z \in \mathbb{R}$ m/s² (raw akselerometer); $\omega_x, \omega_y, \omega_z \in \mathbb{R}$ °/s (raw giroskop)
...
### A — Keterangan Variabel

| Simbol | Tipe | Domain | Satuan | Deskripsi |
|---|---|---|---|---|
| $a_x, a_y, a_z$ | Input | $\mathbb{R}$ | m/s² | Komponen akselerasi raw dari MPU-6050 |
| $\mathbf{b}_{acc} = [b_{ax}, b_{ay}, b_{az}]^T$ | Konstanta (diukur boot) | $\mathbb{R}^3$ | m/s² | **[BARU v9]** Vektor bias akselerometer: offset DC pabrik MPU-6050, diukur sekali oleh A.EKF.0 |
| $\tilde{\mathbf{a}}_t = \mathbf{a}_t - \mathbf{b}_{acc}$ | Intermediat | $\mathbb{R}^3$ | m/s² | **[BARU v9]** Akselerometer terkalibrasi: menggantikan $\mathbf{a}_t$ di A.EKF.3 dan A.EKF.4 |
| $\boldsymbol{\omega}_t = [\omega_x, \omega_y, \omega_z]^T$ | Input | $\mathbb{R}^3$ | °/s | Laju rotasi raw dari giroskop MPU-6050 (termasuk komponen yaw $\omega_z$; nama lama $g_z$ sudah diseragamkan ke $\omega_z$ di v9.1 agar simetris dengan ketiga sumbu) |
...
| $\omega_z^{corr}$ | **Output** | $\mathbb{R}$ | °/s | **Yaw rate terkalibrasi bias** — tersedia untuk post-MVP (navigasi berbasis heading); **tidak** digunakan Formula I atau K di v9.4 (I hanya memakai data YOLO/ToF; K hanya memakai $z_{r,j}$, $t_{disc}$, $B_{cam}$) |
...
CF menggunakan aproksimasi skalar: $\|\mathbf{a}_{lin}\| = |\|\mathbf{a}_t\| - g|$. Ini tidak akurat saat sensor miring — jika kacamata miring 30°, komponen gravitasi di setiap sumbu berbeda dan tidak bisa dieliminasi hanya dengan mengurangi $g = 9.81$ dari norma total. EKF menghitung $\mathbf{g}_q$ — vektor gravitasi yang tepat berdasarkan orientasi quaternion saat ini — lalu mengurangkannya dari $\tilde{\mathbf{a}}_t$ secara vektorial.

**[BARU v9] Mengapa $\|\mathbf{a}_{lin}\|$ tidak pernah nol meskipun device diam — dan bagaimana A.EKF.0 memperbaikinya:**

MPU-6050 memiliki bias offset pabrik per-sumbu yang tidak nol, umumnya 10–80 mg per sumbu (InvenSense datasheet Rev 3.4, §3.1). Bias ini tidak dapat dipelajari EKF karena tidak masuk dalam state vector — EKF hanya memodelkan bias giroskop $\mathbf{b}_\omega$. Akibatnya, inovasi $\mathbf{y}_t = \mathbf{a}_t - h(\hat{\mathbf{x}})$ tidak pernah mendekati $\mathbf{0}$ saat diam: ada residual konstan yang proporsional terhadap besar bias. Secara kuantitatif, jika ketiga sumbu masing-masing memiliki bias $\sim\!25\ \text{mg}$:

$$\|\mathbf{a}_{lin}\|_{bias} \approx \sqrt{3} \times 0.025 \times 9.81 \approx 0.43\ \text{m/s}^2$$

Nilai ini konsisten dengan pembacaan yang diamati pada firmware ($\sim\!0.4\ \text{m/s}^2$). Prosedur A.EKF.0 menghilangkan komponen ini sebelum memasuki loop, sehingga $\|\mathbf{a}_{lin}\|$ saat diam menjadi $< 0.05\ \text{m/s}^2$ — setara dengan noise floor akselerometer pada bandwidth 21 Hz.
...
Prosedur A.EKF.0 sudah dijalankan sebelum langkah ini, sehingga $\mathbf{b}_{acc}$ sudah tersedia. Inisialisasi menggunakan pembacaan akselerometer **terkalibrasi** $\tilde{\mathbf{a}}_0 = \mathbf{a}_0 - \mathbf{b}_{acc}$ agar orientasi awal tidak menyimpang akibat bias pabrik:

$$\theta_0 = \arctan\!\left(\frac{\tilde{a}_{y,0}}{\sqrt{\tilde{a}_{x,0}^2 + \tilde{a}_{z,0}^2}}\right), \quad \phi_0 = \arctan\!\left(\frac{-\tilde{a}_{x,0}}{\tilde{a}_{z,0}}\right)$$
...
[MPU-6050 via I2C @ 100Hz]
    ...
    └──→ ω_x, ω_y, ω_z (°/s)    ← giroskop raw
              │
    [A.0: Guard Δt = max(Δt, 0.001s)]
              │
    [A.EKF.0: KALIBRASI STATIS (boot phase, sekali jalan)]    ← BARU v9
    b_acc = ā × (1 - g/‖ā‖),  ā = rata-rata n_cal=500 sampel
    ã_t = a_t - b_acc  ← digunakan menggantikan a_t di bawah
              │
    ┌─────────┴──────────────────────────────────┐
    ▼                                            ▼
[A.EKF.2: FASE PREDIKSI]              [Simpan sebagai pengukuran]
x̂_{t|t-1} = f(x̂_{t-1}, ω_t, Δt)     ã_t = [ax-bax, ay-bay, az-baz]
P_{t|t-1} = F_t P_{t-1} F_t^T + Q
(quaternion dirotasi, kovarians naik)
    │
    ▼
[A.EKF.3: FASE KOREKSI]
y_t = ã_t - h(x̂_{t|t-1})       ← inovasi (pakai ã_t, bukan a_t)  [v9]
...
[A.EKF.4: EKSTRAKSI OUTPUT dari x̂_{t|t} = [qw,qx,qy,qz, bx,by,bz]]
    ...
    ├──→ ω_z^corr = ω_z - b_{ωz}^(t)  ← yaw rate terkalibrasi
    └──→ ‖a_lin‖ = ‖ã_t - g_q‖        ← pakai ã_t terkalibrasi   [v9]
              │
...
$$z_{r,j}^{valid} = \begin{cases} z_{r,j}, & z_{r,j} \in [\varepsilon_{noise},\ d_{max}] \\ \varnothing, & \text{otherwise} \end{cases}$$

Batas bawah filter menggunakan $\varepsilon_{noise} = 30\ \text{mm}$ — simbol yang sama dengan noise floor ToF di Konstanta Sistem. Nilai ini bukan angka arbitrer: ini adalah batas terendah jarak yang dapat dibaca sensor VL53L5CX secara andal; pembacaan di bawahnya hampir selalu merupakan sentinel error, bukan jarak nyata.
...
$$r_{center}(\theta) = \text{round}\!\left(3.5 + \frac{\theta}{\delta\theta}\right)$$

dengan $\delta\theta = 5.625°/\text{baris}$ (dari Konstanta Sistem). Ekspresi ini menggunakan simbol $\delta\theta$ agar konsisten dengan definisi global dan portabel jika spesifikasi sensor berubah.
...
**Domain input:** $z_{r,j} \in [0,\ d_{max}] \cup \{65535\}$ mm (65535 = sentinel error sensor VL53L5CX), $j \in \{0,...,7\}$, $\theta \in [-90°, +90°]$

**Range output:** $d_{obj} \in [\varepsilon_{noise},\ d_{max}]$ mm — nilai 0 tidak muncul setelah filter E.0; jika semua baris invalid, fallback ke $d_{max} = 4000$ mm

**Contoh nilai $\mathcal{R}_{obj}$ pada berbagai postur kepala:**

| Postur kepala | $\theta$ | $r_{center}$ | $\mathcal{R}_{obj}$ |
|---|---|---|---|
| Kepala tegak (standar) | $0°$ | $\text{round}(3.5 + 0) = 4$ | $\{3, 4, 5\}$ |
| Menunduk 5.7° **(≈ +δθ)** | $+5.7°$ | $\text{round}(4.513) = 5$ | $\{4, 5, 6\}$ |
| Mendongak 5.7° **(≈ −δθ)** | $-5.7°$ | $\text{round}(2.487) = 2$ | $\{1, 2, 3\}$ |
| Menunduk 11.3° **(≈ +2δθ)** | $+11.3°$ | $\text{round}(5.509) = 6$ | $\{5, 6, 7\}$ |

> **[v9.3] Catatan koreksi:** Contoh baris kedua dan ketiga diubah dari $\theta = \pm 5.6°$ menjadi $\theta = \pm 5.7°$. Nilai $5.6°$ berada sedikit di bawah satu langkah penuh $\delta\theta = 5.625°/\text{baris}$, sehingga $3.5 + 5.6/5.625 = 4.496$ yang dibulatkan menghasilkan $r_{center} = 4$ — bukan 5 seperti yang tertulis di versi sebelumnya. Nilai $\theta = 5.7°$ dipilih karena $3.5 + 5.7/5.625 = 4.513 \Rightarrow \text{round}(4.513) = 5$ secara deterministik, dan cukup dekat dengan $\delta\theta$ sehingga tetap representatif sebagai contoh "satu baris geser". Demikian pula baris mendongak: $\theta = -5.7°$ menghasilkan $\text{round}(2.487) = 2$ sehingga $\mathcal{R}_{obj} = \{1, 2, 3\}$ — bukan $\{2, 3, 4\}$ seperti sebelumnya.

### E — Keterangan Variabel

| Simbol | Tipe | Domain | Satuan | Deskripsi |
|---|---|---|---|---|
| $z_{r,j}$ | Input | $[0, d_{max}] \cup \{65535\}$ | mm | Pembacaan sensor ToF pada baris $r$, kolom $j$ (65535 = sentinel error) |
...
| $d_{obj}$ | **Output** | $[\varepsilon_{noise}, d_{max}]$ | mm | **Jarak objek yang terdeteksi** |
...
Saat pengguna menunduk sebesar $\theta > 0$, seluruh sensor ikut menunduk, sehingga baris yang sebelumnya mengarah ke perut objek ($r=4$) kini mengarah ke lantai. Formula E.1 mengkompensasi ini dengan menggeser $r_{center}$ ke bawah ($r$ lebih besar, karena $\theta/\delta\theta > 0$ menambah hasil `round`) agar window pengukuran tetap membaca area badan objek — bukan lantai.
...
Nilai ini sudah dihitung di ESP32 oleh Formula A.EKF.4: $\|\mathbf{a}_{lin}\| = \|\tilde{\mathbf{a}}_t - \mathbf{g}_q\|$ — menggunakan akselerometer terkalibrasi $\tilde{\mathbf{a}}_t$ sesuai perubahan v9.
...
Kecepatan pendekatan per-objek dihitung dengan definisi yang sama dengan Formula G.2, menggunakan $v_{head}^{(i)} = v_{head\_base} \times d_{obj}^{(i)}$ yang berbeda untuk setiap tracking ID. $\Delta t$ yang digunakan di sini adalah nilai yang **sudah melalui saturasi** $\text{sat}(\Delta t_{raw}, \Delta t_{min}, \Delta t_{max})$ dari G.0 — F.3b dan G.0 mengasumsikan $\Delta t$ yang sama dalam satu frame processing:

$$v^{(i)} = \frac{\max\!\left(\Delta d^{(i)},\ 0\right)}{\Delta t} - v_{head}^{(i)}$$
...
> **Mengapa F.3b tidak menggunakan moving average seperti G.2b?** Formula G.2b menerapkan rata-rata 3-frame pada kecepatan untuk meredam *alert flickering* akibat jitter WiFi — prioritasnya adalah **stabilitas threshold** yang menjaga pengalaman pengguna konsisten. Formula F.3b sebaliknya bertujuan menghasilkan *danger score* yang paling merepresentasikan kondisi *saat ini* — responsivitas real-time lebih penting daripada kehalusan. Jika motor mendekat tiba-tiba, F.3b harus langsung mencerminkan kecepatan tinggi itu di frame yang sama, bukan setelah rata-rata 3 frame. Dengan kata lain: G mengutamakan konsistensi peringatan; F mengutamakan deteksi ancaman sesegera mungkin.
...
- **MEDIUM** ($0.35 < D \le 0.70$): Objek dialirkan ke Formula G **dan H** dengan throttle TTS via $f_{tts}$ (minimum 1 detik antar peringatan, dari Konstanta Sistem); TTS priority normal. Formula H dicek untuk kondisi objek statis one-shot.
...
**F.5 — Output ke TTS Layer (BARU v9.2):**

Pool yang sudah terbentuk di F.4 dikonversi menjadi token TTS dengan batas kapasitas per frame. Ini mencegah antrean audio meledak saat scene sangat ramai (misalnya persimpangan dengan $>$5 objek aktif):

$$\boxed{N_{alert} = \min\!\left(|\text{pool}_{HIGH}| + |\text{pool}_{MEDIUM}|,\ N_{obj,max}\right)}$$

Prioritas pengurutan jika $|\text{pool}_{HIGH}| + |\text{pool}_{MEDIUM}| > N_{obj,max}$: objek HIGH selalu diprioritaskan; objek MEDIUM diurutkan berdasarkan $D^{(i)}$ menurun dan dipotong di posisi $N_{obj,max}$.

Throttle untuk pool MEDIUM:

$$\delta_{tts}^{(i)} = \begin{cases}
1, & \text{pool}^{(i)} = \text{HIGH} \\
1, & \text{pool}^{(i)} = \text{MEDIUM}\ \wedge\ (t - t_{last\_tts}^{(i)}) \ge 1/f_{tts} \\
0, & \text{otherwise}
\end{cases}$$

di mana $t_{last\_tts}^{(i)}$ adalah timestamp terakhir TTS untuk objek ke-$i$, dan $f_{tts} = 1\ \text{Hz}$ adalah laju maksimum output TTS untuk kategori MEDIUM (dari Konstanta Sistem). Nilai $f_{tts}$ dipilih empiris: lebih dari 1 Hz membuat pengguna kewalahan dengan audio yang tumpang-tindih; kurang dari 1 Hz membuat jeda terlalu panjang untuk objek yang mendekat dengan kecepatan sedang.

> **Mengapa HIGH dikecualikan dari throttle?** Objek dengan $D > D_{HIGH}$ sudah masuk zona kritis (jarak $< d_{crit}$ atau kecepatan tinggi). Menunda TTS-nya bahkan 1 detik berpotensi fatal. Throttle adalah mekanisme kenyamanan pengguna, bukan mekanisme keselamatan — keduanya tidak boleh dicampur.
...
| Simbol | Tipe | Domain | Satuan | Deskripsi |
|---|---|---|---|---|
| $\|\mathbf{a}_{lin}\|$ | Input | $[0, \infty)$ | m/s² | Magnitude akselerasi linear dari **MPU-6050 via paket WebSocket** (sudah dikurangi gravitasi oleh Formula A.EKF.4) |
| $a_{th}$ | Konstanta | — | m/s² | Threshold gerakan = 2.94 m/s² ($\approx 0.3g$) |
| $d_{obj}^{(i),(t)}$ | Input | $[\varepsilon_{noise}, d_{max}]$ | mm | Jarak objek ke-$i$, frame saat ini (dari Formula E, per tracking ID; nilai 0 tidak muncul setelah filter E.0) |
| $d_{obj}^{(i),(t-1)}$ | State | $[\varepsilon_{noise}, d_{max}]$ | mm | Jarak objek ke-$i$, frame sebelumnya |
...
**G.0 — Guard Interval Waktu WebSocket** *(dari timestamp ESP32):* **[v9.2: judul eksplisit]**

$$\Delta t_{raw} = \frac{ts_{esp}^{(t)} - ts_{esp}^{(t-1)}}{1000}$$

$$\boxed{\Delta t = \text{sat}\!\left(\Delta t_{raw},\ \Delta t_{min},\ \Delta t_{max}\right)}$$

Nilai $\Delta t_{min} = 0.001\ \text{s}$ dan $\Delta t_{max} = 0.5\ \text{s}$ mengacu ke Konstanta Sistem — **bukan angka hardcoded di sini**. Konsistensi ini penting agar jika nilai batas diubah saat tuning lapangan, satu perubahan di tabel konstanta langsung berlaku untuk Formula G dan semua formula yang merujuknya.
...
dengan $\|\mathbf{P}_t\|_F = \sqrt{\sum_{i,j} P_{ij}^2}$ adalah norma Frobenius matriks kovarians EKF yang **dihitung di ESP32** — hasilnya tidak dikirim mentah; ESP32 hanya mengirim flag boolean `is_converged` (lihat A.6) setelah mengevaluasi kondisi OR di A.EKF.5. Mobile tidak pernah menerima nilai $\|\mathbf{P}_t\|_F$ secara langsung. Dan $\varepsilon_{conv} = 0.10$.
...
$$v_{raw}^{(i)} = \frac{\max\!\left(d_{obj}^{(i,t-1)} - d_{obj}^{(i,t)},\ 0\right)}{\Delta t} - v_{head}^{(i)}$$

dengan hasil diklamping ke nol jika negatif: $v_{raw}^{(i)} := \max(v_{raw}^{(i)}, 0)$. Formula ini berjalan per-objek (per tracking ID), konsisten dengan G.1b yang mendefinisikan $v_{head}^{(i)}$ dan $d_{obj}^{(i)}$.
...
### G — Keterangan Variabel

| Simbol | Tipe | Domain | Satuan | Deskripsi |
|---|---|---|---|---|
| $d_{obj}^{(t)}$ | Input | $[\varepsilon_{noise}, d_{max}]$ | mm | Jarak objek frame saat ini (dari Formula E; nilai 0 tidak muncul setelah filter E.0) |
| $d_{obj}^{(t-1)}$ | State | $[\varepsilon_{noise}, d_{max}]$ | mm | Jarak objek frame sebelumnya (disimpan di memori) |
...
| $v_{head}^{(i)}$ | Intermediat (Mobile) | $[0, \infty)$ | mm/s | Kecepatan semu per-objek: $v_{head\_base} \times d_{obj}^{(i)}$ — satu perkalian per tracking ID. Satuannya mm/s karena rad bersifat dimensionless: $\frac{\text{rad}}{\text{s}} \times \text{mm} = \frac{\text{mm}}{\text{s}}$ |
...
> **[v9.3] Catatan penambahan kolom $d_{obj}^{(i)}$:** Kolom ini ditambahkan agar nilai $v_{head}^{(i)} = v_{head\_base} \times d_{obj}^{(i)}$ dapat diverifikasi secara independen oleh pembaca. Setiap baris menggunakan $d_{obj}$ yang berbeda karena ini adalah skenario per-objek: baris kepala menunduk menggunakan $d_{obj} = 1500\ \text{mm}$ (objek di depan pada jarak sedang), baris jalan menggunakan $d_{obj} = 2500\ \text{mm}$ (objek yang jauh) atau $d_{obj} = 950\ \text{mm}$ (objek dekat), konsisten dengan nilai $v_{head}$ yang tercantum di kolom berikutnya.
...
> **Catatan:** Setiap baris di bawah adalah **skenario per-objek independen** dengan nilai $d_{obj}^{(i)}$ yang berbeda — bukan satu tracking ID yang sama dalam satu frame. Lihat kolom $d_{obj}^{(i)}$ untuk memverifikasi nilai $v_{head}^{(i)}$ di kolom berikutnya.

| Skenario | $\theta$ | $\omega_x^{corr}$ | $k_{damp}$ | $d_{obj}^{(i)}$ | $v_{head}^{(i)}$ | $v_{raw}$ | $\bar{v}$ | $T$ | Objek di | Peringatan? |
|---|---|---|---|---|---|---|---|---|---|---|
| Diam, kepala diam | 0° | 0 °/s | 1.0 | 2500 mm | 0 mm/s | 0 | 0 mm/s | 1000 mm | 2500 ≥ 1000 | ❌ |
| Diam, kepala menunduk 10°/s (θ=0°) | 0° | 10 °/s | 0.5 | 1500 mm | 131 mm/s | 0 (clamped) | 0 mm/s | 1000 mm | 2500 ≥ 1000 | ❌ ✅ |
| Diam, kepala menunduk 10°/s (θ=30°) | 30° | 10 °/s | 0.5 | 1500 mm | 113 mm/s | 0 (clamped) | 0 mm/s | 1000 mm | 2500 ≥ 1000 | ❌ ✅ |
| Jalan cepat 1500 mm/s, kepala diam | 0° | 0 °/s | 1.0 | 2500 mm | 0 mm/s | 1500 | 1500 mm/s | 4000 mm | 2500 < 4000 | ✅ |
| Jalan 500 mm/s, menunduk 20° | 20° | 2 °/s | 1.0 | 950 mm | 31 mm/s | 469 | ~490 mm/s | ~1980 mm | 950 < ~1980 | ✅ |
...
> **[v9.2] Independensi Formula H terhadap EKF Warmup:** Flag `alerted[i]` (representasi runtime dari $f_{obj}$) **tidak** direset selama periode EKF warmup (`is_converged = 0`). Ini keputusan arsitektur yang disengaja: Formula H adalah *one-shot guard* berbasis jarak absolut ($d_{obj} < d_{w0}$) — operasinya tidak bergantung pada kecepatan pendekatan maupun kompensasi gerak kepala yang memerlukan EKF konvergen. Jika flag H direset bersamaan dengan reset EKF (misalnya saat device di-reboot), objek statis berbahaya yang sudah ada di sekitar pengguna saat boot akan disuarakan ulang tidak perlu — bukan masalah keselamatan, tetapi mengganggu. Yang berbahaya adalah sebaliknya: flag H *tidak* di-set untuk objek baru yang muncul saat warmup. Ini tidak terjadi karena Formula H berjalan independen dari `is_converged`.

### H — Keterangan Variabel
...
$$\text{if } y_{max}^{(t)} = y_{min}^{(t)} \text{ OR } x_{max}^{(t)} = x_{min}^{(t)}: \quad \lambda^{(t)} := 1.0,\ \text{ar\_score} := 1.0 \quad \text{(skip I.3 — degenerate BBox)}$$
...
>
> **[DIPERBARUI v8]** Model diperluas dari dua zona vertikal menjadi **empat zona** ($z_{high}$, $z_{mid}$, $z_{low}$, $z_{ultra}$); enam fitur spasial-temporal diekstrak; klasifikasi output diperluas dari binary menjadi **tujuh tipe terrain**: OPEN, CONTAMINATED, STAIR\_DOWN, HOLE, STAIR\_UP, RAMP, SAFE; ditambahkan estimasi kedalaman/ketinggian anomali dalam mm; penambahan deteksi arah spasial kompatibel dengan sistem jam Formula C; confidence scoring per deteksi memungkinkan routing bertingkat ke Formula G/H.
...
**Guard non-overlap** (wajib setelah `round`, sebelum J.1): baris zona tidak boleh tumpang-tindih. Setelah keempat `round` di atas dihitung, evaluasi berurutan dari atas ke bawah:

$$r_{mid} := \max(r_{mid},\ r_{high} + 1)$$
$$r_{low} := \max(r_{low},\ r_{mid} + 1)$$
$$r_{ultra} := \max(r_{ultra},\ r_{low} + 1)$$

Hasil akhir tetap di-`sat()` ke range masing-masing. Jika `sat()` memaksa dua zona ke baris yang sama (misalnya $\theta$ sangat ekstrim), zona bawah diprioritaskan karena data terrain dekat kaki lebih kritis untuk keselamatan.

**J.0b — Filter sentinel sebelum rata-rata zona:**

$$z_{r,j}^{J} = \begin{cases} z_{r,j}, & z_{r,j} \in [\varepsilon_{noise},\ d_{max}] \\ d_{max}, & \text{otherwise (sentinel 0 atau 65535)} \end{cases}$$

Nilai $z_{r,j} = 0$ (dropout sensor) dan $z_{r,j} = 65535$ (sentinel VCSEL error) di-replace dengan $d_{max}$ sebelum masuk ke rata-rata zona. Ini mencegah nilai ekstrem menghasilkan $\bar{z}$ yang tidak merepresentasikan terrain nyata dan memicu klasifikasi STAIR/HOLE palsu.

**J.1 — Rata-rata jarak empat zona vertikal:**

$$\bar{z}_{high} = \frac{1}{N_{col}} \sum_{j=0}^{N_{col}-1} z_{r_{high}(\theta),\, j}$$

$$\bar{z}_{mid} = \frac{1}{2} \sum_{r \in \{r_{mid},\ \text{sat}(r_{mid}+1,\, r_{mid},\, r_{low}-1)\}} \frac{1}{N_{col}}\sum_{j=0}^{N_{col}-1} z_{r,j}^{J}$$

> **Guard overlap z̄_mid:** Baris kedua di-sat ke range $[r_{mid}, r_{low}-1]$ agar tidak pernah memasuki zona $r_{low}$. Saat $r_{mid} = 4$ (maksimum sat), ekspresi menjadi $\text{sat}(5,\, 4,\, r_{low}-1) = r_{low}-1 = 4$, sehingga kedua baris identik dan z̄_mid efektif dihitung dari satu baris saja — tetap valid secara matematis.


$$\bar{z}_{low} = \frac{1}{N_{col}} \sum_{j=0}^{N_{col}-1} z_{r_{low}(\theta),\, j}$$

$$\bar{z}_{ultra} = \frac{1}{2} \sum_{r \in \{r_{ultra}, r_{ultra}-1\}} \frac{1}{N_{col}}\sum_{j=0}^{N_{col}-1} z_{r,j}^{J}$$
...
Nilai positif besar menunjukkan lantai dekat "menghilang" - pengguna semakin mendekati tepi atau lubang. **Penggunaan di J.3 & J.6:** $\Delta z_t$ berfungsi sebagai modifier confidence untuk HOLE — jika $\Delta z_t > \Delta z_{step}$ (lantai menghilang cepat) maka confidence HOLE ditingkatkan 10% di J.6, membantu membedakan kasus pengguna mendekati lubang dengan cepat dari noise pengukuran biasa.
...
> **[v9.4] Catatan $N_{row,low}$:** Zona $r_{low}$ merujuk tepat satu baris (baris 5, lihat J.0), sehingga $N_{row,low} = 1$ secara implisit — formula ekuivalen dengan $\sigma_j = |z_{r_{low},j} - \bar{z}_{low}|$, bukan standar deviasi multi-sampel. Simbol $N_{row,low}$ dipertahankan untuk portabilitas jika zona $r_{low}$ kelak diperluas.
...
\text{CONTAMINATED},    & \bar{z}_{low} < d_{cont} \\
...
**Hysteresis tetap dipertahankan:** Untuk mencegah *alert flapping* pada kondisi borderline (R berfluktuasi di sekitar $R_{th,hi}$), logika hysteresis v7 tetap aktif sebagai post-processing: jika type berganti-ganti antara STAIR\_DOWN dan SAFE dalam 3 frame berturut-turut tanpa stabilisasi, pertahankan state sebelumnya.

> **Catatan alur data — peran $h_{est}$ dan $\text{dir}$:** Kedua variabel ini adalah **output langsung ke layer TTS**, bukan input numerik ke formula lain. $h_{est}$ digunakan untuk mengisi template pesan suara ("Tangga turun sekitar *[h_est]* sentimeter") dan $\text{dir}$ untuk mengisi arah ("di *[dir]*"). Tidak ada formula di pipeline (G, H, I, K) yang menerima $h_{est}$ atau $\text{dir}$ sebagai operand perhitungan — keduanya murni informasi kontekstual untuk pengguna. Ini berbeda dari $\delta_J$ yang merupakan sinyal biner yang secara aktif mempengaruhi keputusan routing Formula G.

---

### J — Keterangan Variabel

| Simbol | Tipe | Domain | Satuan | Deskripsi |
|---|---|---|---|---|
| $z_{r,j}$ | Input | $[0, d_{max}] \cup \{65535\}$ | mm | Data ToF matriks 8x8 dari sensor VL53L5CX (65535 = sentinel error) |
| $\theta$ | Input | $[-90°, +90°]$ | ° | Pitch kepala dari Formula A (via paket WebSocket) |
...
| $\text{type}$ | Intermediat | $\{\text{OPEN, CONTAMINATED, STAIR\_DOWN, STAIR\_UP, HOLE, RAMP, SAFE}\}$ | — | Tipe terrain terklasifikasi |
...
ELIF type ∈ {STAIR_DOWN, HOLE, STAIR_UP} AND C_MID < C ≤ C_HIGH:
    -> delta_J = 1 (MED): Feed ke Formula G dan H sebagai confirmation
...
**K.1 — Himpunan piksel valid:**

$$\mathcal{V}_K = \left\{(r,j) : z_{r,j} \in [\varepsilon_{noise},\ d_{max}]\right\}$$

Nilai $z_{r,j} = 0$ (dropout sensor) dan $z_{r,j} = 65535$ (sentinel error VCSEL/ambient) dibuang untuk mencegah buzzer palsu.

**K.1b — Jarak minimum zona:**

$$d_{min} = \begin{cases} \displaystyle\min_{(r,j) \in \mathcal{V}_K} z_{r,j}, & \mathcal{V}_K \neq \varnothing \\ d_{max}, & \mathcal{V}_K = \varnothing \end{cases}$$
...
**Domain:** $d_{min} \in [\varepsilon_{noise},\ d_{max}]$ mm (nilai 0 dan 65535 dibuang oleh K.1; fallback ke $d_{max}$ jika semua piksel error), $\delta_{buzz} \in \{0,1\}$

### K — Keterangan Variabel

| Simbol | Tipe | Domain | Satuan | Deskripsi |
|---|---|---|---|---|
| $z_{r,j}$ | Input | $[0, d_{max}] \cup \{65535\}$ | mm | Seluruh 64 zona ToF (65535 = sentinel error, dibuang oleh K.1) |
| $d_{min}$ | Intermediat | $[\varepsilon_{noise}, d_{max}]$ | mm | Jarak terdekat dari zona valid $\mathcal{V}_K$ (fallback ke $d_{max}$ jika semua piksel error) |
...
|---|---|---|---|---|---|---|
| **A** | Filter IMU (EKF) | EKF quaternion: prediksi giroskop + koreksi akselerometer + estimasi bias | Raw MPU-6050 6-axis | $\theta, \phi, \omega_x^{corr}, \omega_y^{corr}, \omega_z^{corr}, \|\mathbf{a}_{lin}\|, ts_{esp}, v_{head\_base}, \text{is\_converged}$ **(9 field — A.6)** | Estimasi orientasi kepala adaptif + pra-komputasi $v_{head\_base}$ + flag konvergensi EKF | **ESP32-S3** |
...
| **G** | Threshold Adaptif | $T = \min(d_{w0}+v \cdot t_r,\ d_{max})$; $\Delta t=\text{sat}(\Delta t_{raw}, \Delta t_{min}, \Delta t_{max})$; guard frame-1 | $d_{obj}^{(t)}, d_{obj}^{(t-1)}, ts_{esp}, v_{head\_base}, \text{is\_converged}$ | $\delta_G \in \{0,1\}$ | Peringatan dini berbasis kecepatan | Smartphone |
...
| **J** | Klasifikasi Terrain | 4-zona vertikal; 6 fitur ($\Delta z_v$, $\Delta z_t$, $\sigma_j$, $R$, $\xi$, pattern); decision tree terrain | $z_{r,j}$, $\theta$ | type $\in$ {OPEN/CONTAM/DOWN/UP/HOLE/RAMP/SAFE}, $h_{est}$ (mm), dir (jam), $C \in [0,1]$, $\delta_J$ | Klasifikasi 7 tipe terrain + estimasi kedalaman + arah | Smartphone |
...
═══════════════════════════════════════════════════════════════════════════
  ...
  y_min, y_max          via I2C @ 30Hz          ωx,ωy,ωz (°/s)
  via YOLOv11           ─────────────          ──────────────────────
  ─────────────               │                          │
        │                     │                          │
        │                     │           ┌──────────────┘
        ...
        │                     │   │ θ, φ, ω_x^corr, ω_y^corr,    │
        │                     │   │ ω_z^corr, ‖a_lin‖, ts_esp,    │
        │                     │   │ v_head_base, is_converged      │
        │                     │   │                               │
        │                     │   │ [Formula K: Fail-Safe]        │
        │                     │←──┤ d_min → buzzer GPIO           │
        ...
        │                     │                  │
        │                     │         [WebSocket JSON @30Hz]
        │                     │         {θ, φ, ω_x^corr, ω_y^corr, ω_z^corr, ‖a_lin‖, ts_esp, v_head_base, is_converged}
        │                     │                  │
        │                     │                  │
═══════╪═════════════════════╪══════════════════╪══════════════════════
...
═══════════════════════════════════════════════════════════╪═════════════════╪═══
  ...
  [OV2640, BBox area t & t-1 & t-2, class label]
         │
         ▼
  [Formula I: TTC Multi-Dimensional (v8)]
  area_score  ← ΔA / ΔA_norm
  ar_score    ← stabilitas λ = w/h
  dist_score  ← konsistensi ToF vs area
  TTC_score   = w_A·area + w_AR·ar + w_dist·dist
  TTC_weighted = min(TTC_score × m_class, 1.0)
         │
    ┌────┼──────────────┐
    ▼    ▼              ▼
IMMINENT PROBABLE   POSSIBLE
Override Feed ke G   Silent
G+H      + H         (log)
 ↓ TTS    ↓ TTS
"Kendaraan  "Objek
 mendekat!"  mendekat"
